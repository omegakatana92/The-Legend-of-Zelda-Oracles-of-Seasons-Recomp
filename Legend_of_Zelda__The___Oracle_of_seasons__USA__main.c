/* Main entry point */
#include "Legend_of_Zelda__The___Oracle_of_seasons__USA_.h"
#include "gbrt.h"
...
#ifdef _WIN32
static uint8_t* gb_prompt_for_rom(const char* title, size_t* out_size) {
    *out_size = 0;
    char filename[MAX_PATH] = {0};
    OPENFILENAMEA ofn = {0};
    ofn.lStructSize = sizeof(ofn);
    ofn.hwndOwner = NULL;
    ofn.lpstrFilter = "Game Boy ROMs (*.gb;*.gbc;*.bin)\0*.gb;*.gbc;*.bin\0All Files (*.*)\0*.*\0";
    ...
    if (!f) {
        MessageBoxA(NULL, "Failed to open ROM file.", "Error", MB_ICONERROR);
        return NULL;
    }
    fseek(f, 0, SEEK_END);
    long size = ftell(f);
    if (size <= 0 || size > 32 * 1024 * 1024) {
        fclose(f);
        MessageBoxA(NULL, "Invalid ROM file size.", "Error", MB_ICONERROR);
        return NULL;
    }
    fseek(f, 0, SEEK_SET);
    ...
    if (!data) {
        fclose(f);
        MessageBoxA(NULL, "Failed to allocate memory for ROM.", "Error", MB_ICONERROR);
        return NULL;
    }
    if (fread(data, 1, (size_t)size, f) != (size_t)size) {
        free(data);
        fclose(f);
        MessageBoxA(NULL, "Failed to read ROM file.", "Error", MB_ICONERROR);
        return NULL;
    }
    fclose(f);
...
int Legend_of_Zelda__The___Oracle_of_seasons__USA__main(int argc, char* argv[]) {
    ...
    GBConfig runtime_config = *Legend_of_Zelda__The___Oracle_of_seasons__USA__default_config();
    ...
    if (differential_mode) {
        ...
        gb_context_set_save_id(generated_ctx, "Legend_of_Zelda__The___Oracle_of_seasons__USA_");
        gb_context_set_save_id(interpreted_ctx, "Legend_of_Zelda__The___Oracle_of_seasons__USA_");
        if (serial_stdout) {
            generated_ctx->callbacks.on_serial_byte = gbrt_serial_stdout;
        }
        Legend_of_Zelda__The___Oracle_of_seasons__USA__init(generated_ctx);
        Legend_of_Zelda__The___Oracle_of_seasons__USA__init(interpreted_ctx);
        if (data_mod_file) {
    ...
    gb_context_set_save_id(ctx, "Legend_of_Zelda__The___Oracle_of_seasons__USA_");
...
#ifdef _WIN32
    uint8_t* custom_rom = NULL;
    size_t custom_rom_size = 0;
    custom_rom = gb_prompt_for_rom("Select Legend of Zelda, The - Oracle of Seasons ROM", &custom_rom_size);
#endif
    Legend_of_Zelda__The___Oracle_of_seasons__USA__init(ctx);
#ifdef _WIN32
    if (custom_rom && custom_rom_size > 0) {
        gb_context_load_rom(ctx, custom_rom, custom_rom_size);
        ctx->mbc_type = custom_rom[0x147];
    ...
    const GBPortMetadata port_metadata = {
        ...
        .game_id = "Legend_of_Zelda__The___Oracle_of_seasons__USA_",
        .game_title = "Legend_of_Zelda__The___Oracle_of_seasons__USA_",
        .rom_sha256 = "862a51368fb30539279d336b3fe193b43876d2cb15c87a36f5da517804ab3971",
        .rom_size = 1048576u,
    };
    ...
    Legend_of_Zelda__The___Oracle_of_seasons__USA__run(ctx);
...
int main(int argc, char* argv[]) {
    return Legend_of_Zelda__The___Oracle_of_seasons__USA__main(argc, argv);
}
